...
class ClassScheduleController extends Controller
{
    ...
    public function store(Request $request, Section $section)
    {
            ...
            'subject' => [
                ...
                Rule::unique('class_schedules')
                    ->where(function ($query) use ($teacher, $request) {
                        // Ensure no duplicate schedule by the same teacher
            ...
            'subject_code'  => [
                ...
                Rule::unique('class_schedules')
                    ->where(function ($query) use ($section, $request) {
                        // Ensure different teachers cannot add the same schedule in the same section
       ...
       $classSchedule = $section->classSchedule()->create(array_merge($validated, [
        ...
        // Attach the days_of_weeks to the pivot table (class_schedule_days)
        $classSchedule->daysOfWeek()->attach($validated['days_of_weeks']);  // daysOfWeek() is the relationship
